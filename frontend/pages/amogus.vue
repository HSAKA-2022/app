    ...
    <div id="app">
      <div v-if="!loading">
        <div v-if="state.isRegistered == false">
          <div class="card" v-if="secret != undefined && !state.alreadyStarted">
            ...
            <footer class="card-footer">
              <button
                class="card-footer-item is-primary button"
                @click="register"
                :disabled="
                  inputs.username == undefined || inputs.username == ''
                "
          ...
          <div
            v-if="secret == undefined && !state.alreadyStarted"
        ...
        <div v-if="state.isAdmin">
          <section class="card my-3">
            ...
            <div class="card-content">
              <div v-if="new Date(state.startingAt) > new Date()">
                ...
                <div class="field">
                  <label class="label"
                    >initial kill cooldown (in seconds)</label
                  >
                  <div class="control">
                    <input
                      class="input"
          ...
          >
            <header class="card-header">
              <p class="card-header-title">Meeting</p>
            </header>
            ...
            <div class="card-content">
              ...
              <ul>
                <li v-for="report in state.reports">
                  ...
                  <span v-if="report.body === undefined">
                    {{ report.reporter }} called an emergency Meeting
                  </span>
                </li>
              </ul>
            </div>
            ...
            <div class="card-content">
              ...
              <div
                ...
                v-for="alive in state.diedToday.sort((a, b) => {
            ...
            <div class="card-content">
              ...
              <div
                class="level"
                style="border-bottom: 1px solid #ebebeb"
                v-for="alive in state.alive.sort((a, b) => {
                  if (a.name > b.name) return 1
                  else if (a.name === b.name) return 0
                  else return -1
                })"
        ...
        <div v-if="!state.isAdmin && state.isRegistered">
          <div v-if="state.isAlive">
            <div
              ...
              v-if="new Date(state.startingAt) > new Date()"
            >
              <div class="message-header">
                <p>Das Spiel startet bald</p>
              </div>
              <div class="message-body">
                Das Spiel startet um
                {{ new Date(state.startingAt).toLocaleString() }} (in ca.
                {{
                  (
                  ...
                  <div class="control">
                    <div class="select" style="width: 100%">
                      <select v-model="inputs.room" style="width: 100%">
                        <option v-for="room in state.rooms" :value="room">
                          {{ room }}
                        </option>
                      </select>
                    </div>
            ...
            <div v-if="new Date(state.startingAt) < new Date()">
              ...
              >
                ...
                <div
                  ...
                  :class="{
                    'is-success':
                      new Date(state.imposter.killCooldown) < new Date(),
                    'is-warning':
                      new Date(state.imposter.killCooldown) > new Date(),
                  }"
                ...
                >
                  <p
                    class="panel-heading level mb-0"
                    v-if="new Date(state.imposter.killCooldown) > new Date()"
                  >
                    Du kannst in
                    {{
                      getTimeDiff(
                        new Date(state.imposter.killCooldown),
                        new Date()
                      )
                    }}
                    ({{
                      new Date(
                        state.imposter.killCooldown
                      ).toLocaleTimeString()
                    }}) wieder töten
                    <span class="icon">
                      <i class="fas fa-times fa-solid"></i>
                ...
                <span
                  ...
                  >Du wirst beim nächsten Morgenplenum sprechen</span
                >
                <button
                  class="button mt-3"
              ...
              <div class="my-6" v-if="state.roomInformation != undefined">
                <h2 class="title is-3">
                  {{ state.roomInformation.room.name }}
                </h2>
              ...
              <p class="mt-4">
                Als Crewmate ist es dein Ziel die Imposter zu finden und im
                Morgenplenum rauszuwählen (und natürlich zu überleben). Um
                jemanden vorschlagen zu können musst du die Überreste eines
                Crewmates finden, indem du die QR Codes in den einzelnen Räumen
                von Burg Fürsteneck regelmäßig scannst. Sollte dort jemand von
                den Impostern eliminiert worden zu sein, kannst du dies melden
                und erhälst dann im nächsten Morgenplenum die Chance der
                versammelten Gemeinschaft jemanden zum rauswählen vorzuschlagen.
                Um das einfacher zu machen, wird dir zudem eine Liste von
                SpielerInnen angezeigt, von denen einer oder eine garantiert der
                Mörder ist. Du solltest also die SpielerInnen auf der Liste zur
                Rede stellen und schauen was sie so gemacht haben in letzter
                Zeit und dann deine Ergebnisse allen vorstellen.
              </p>
              <p class="mt-4">
                Zusätzlich kannst du auch ohne Überreste zu finden jemanden
                vorschlagen, indem du ein Notfall Meeting einberufst. Beachte
                jedoch dass die Imposter ebenfalls Notfall Meetings einberufen
                und Überreste finden können, um die Crewmates auf eine falsche
                Fährte zu locken.
              </p>
              <p class="mt-4">
                Als Imposter hingegen möchtest du möglichst viele QR Codes von
                Crewmates scannen um sie dann zu "töten". Beachte jedoch, dass
                du nicht jederzeit beliebig töten kannst, da es einen Cooldown
                gibt, den du hier angezeigt bekommst. Außerdem möchtest du
                natürlich keine Aufmerksamkeit auf dich ziehen.
              </p>
            </div>
          </div>
          <div v-if="!state.isAlive">
...
export default {
  data() {
    ...
    return {
      scannedUser: new URLSearchParams(window.location.search).get(
        "scannedUser"
      ),
  ...
  methods: {
    ...
    // Admin stuff
    async saveSettings() {
      const result = await fetch(`${SERVER_URL}/${riddleId}/changeSettings`, {
        headers: header(),
        method: "POST",
        body: JSON.stringify({
          ...this.inputs.admin,
    ...
    async voteKick(user) {
      const approval = confirm(
        "Are you sure you want to vote kick " + user.name + "?"
      )

      if (!approval) return

      const result = await fetch(`${SERVER_URL}/${riddleId}/voteKillPlayer`, {
        headers: header(),
        method: "POST",
        body: JSON.stringify({
          playerId: user.id,
    ...
    async endMeeting() {
      ...
      const result = await fetch(`${SERVER_URL}/${riddleId}/closeMeeting`, {
        headers: header(),
        method: "POST",
        body: JSON.stringify({}),
      })
    ...
    async register() {
      ...
      const result = await fetch(`${SERVER_URL}/${riddleId}/register`, {
        headers: header(),
        method: "POST",
        body: JSON.stringify({
    ...
    async reportBody(body) {
      const result = await fetch(`${SERVER_URL}/${riddleId}/reportBody`, {
        headers: header(),
        method: "POST",
        body: JSON.stringify({
          bodyId: body.id,
    ...
    async callEmergencyMeeting() {
      const approval = confirm(
        "Möchtest du im nächsten Morgenplenum einen Slot zum Sprechen haben?"
      )
      if (!approval) return
    ...
    async kill() {
      const result = await fetch(`${SERVER_URL}/${riddleId}/kill`, {
        headers: header(),
        method: "POST",
        body: JSON.stringify({
  ...
  async created() {
    ...
    if (this.state.isAdmin) {
      this.inputs.admin = {
        ...
        killCooldownInSeconds: this.state.killCooldownInSeconds,
        initialKillCooldownInSeconds: this.state.initialKillCooldownInSeconds,
        possibleMurdersPerImposter: this.state.possibleMurdersPerImposter,
      }
    }
    if (this.room != undefined) {
      const result = await fetch(`${SERVER_URL}/${riddleId}/goIntoRoom`, {
        headers: header(),
        method: "POST",
        body: JSON.stringify({
          roomId: this.room,
